...
Planet  *initializePlanets (int nPlanets) {
	...
	for (int i = 0; i < nPlanets; i++) {
		float angle = 	((float) i / nPlanets) * 2.0 * PI +
						((generateRandom() - 0.5) * 0.5);
...
int main(int argc, char **argv) {
	...
	Planet *local_planets =
        (Planet *) malloc(sizeof(*local_planets) * items_per_process);
	...
	if (rank == 0) {
		gathered_planets =
        (Planet *) malloc(sizeof(*gathered_planets) * nPlanets);
	}
	MPI_Gather(
        local_planets,
        items_per_process,
